...
void CreateShaders()
{
	...
	Shader* shader6 = new Shader(); //shader para el color café
...
int main()
{
	...
	while (!mainWindow.getShouldClose())
	{
		...
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(-1.2f, 0.6f, -4.0f)); // Ajustar la posición si es necesario
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		glUniformMatrix4fv(uniformProjection, 1, GL_FALSE, glm::value_ptr(projection));

		meshColorList[0]->RenderMeshColor(); // Índice 0 = Letra M

		// Renderiza la letra V
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(-1.0f, 1.5f, -4.0f)); // Ajustar la posición si es necesario
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		glUniformMatrix4fv(uniformProjection, 1, GL_FALSE, glm::value_ptr(projection));

		meshColorList[1]->RenderMeshColor(); // Índice 1 = Letra V

		// Renderiza la letra G
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(-0.8f, 0.6f, -4.0f)); // Ajustar la posición si es necesario
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		glUniformMatrix4fv(uniformProjection, 1, GL_FALSE, glm::value_ptr(projection));

		meshColorList[2]->RenderMeshColor(); // Índice 2 = Letra G

		//Casa
		//Para el cubo rojo, se utiliza el primer set de shaders con índice 0 en ShaderList
		...
		//Puerta
		//Para el cubo verde, se utiliza el tercer set de shaders con índice 2 en ShaderList
		...
		//Pino 1
		//Para la piramide verde oscuro, se utiliza el quinto set de shaders con índice 4 en ShaderList
		...
		//Tronco 1
		//Para el cubo café, se utiliza el sexto set de shaders con índice 5 en ShaderList
